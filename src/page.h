...
// Forked rx-slow-hash.c directly from Evolution
...
#define EVOLUTIONEXPLORER_RPC_VERSION_MAJOR 2
#define EVOLUTIONEXPLORER_RPC_VERSION_MINOR 6
#define MAKE_EVOLUTIONEXPLORER_RPC_VERSION(major,minor) (((major)<<16)|(minor))
#define EVOLUTIONEXPLORER_RPC_VERSION \
    MAKE_EVOLUTIONEXPLORER_RPC_VERSION(EVOLUTIONEXPLORER_RPC_VERSION_MAJOR, EVOLUTIONEXPLORER_RPC_VERSION_MINOR)
...
struct tx_details
{
    ...
    get_mstch_map() const
    {
        ...
        if (!input_key_imgs.empty())
        {
            ...
            fee_str               = fmt::format("{:0.9f}", evo_amount);
            fee_short_str         = fmt::format("{:0.9f}", evo_amount);
            fee_nano_str          = fmt::format("{:04.0f}", evo_amount * 1e6);
        ...
        mstch::map txd_map {
                ...
                {"sum_inputs"        , evo_amount_to_str(evo_inputs , "{:0.9f}")},
                {"sum_outputs"       , evo_amount_to_str(evo_outputs, "{:0.9f}")},
                {"sum_inputs_short"  , evo_amount_to_str(evo_inputs , "{:0.9f}")},
                {"sum_outputs_short" , evo_amount_to_str(evo_outputs, "{:0.9f}")},
...
index2(uint64_t page_no = 0, bool refresh_page = false)
{
    ...
    std::future<json> network_info_ftr = std::async(std::launch::async, [&]
    {
        ...
        get_evolution_network_info(j_info);
    ...
    while (i >= start_height)
    {
        ...
        if (enable_block_cache && block_tx_cache.Contains(i))
        {
            ...
            for (const pair<crypto::hash, mstch::node> &txd_pair: txd_pairs)
            {
                ...
                if (i + config::tx_settings::EVOLUTION_TX_CONFIRMATIONS_REQUIRED > height)
                {
    ...
    if (CurrentBlockchainStatus::is_thread_running())
    {
        ...
        string emission_coinbase = evo_amount_to_str(current_values.coinbase, "{:0.9f}");
        string emission_fee      = evo_amount_to_str(current_values.fee, "{:0.9f}");
...
show_block(uint64_t _blk_height)
{
    ...
    context["sum_fees"] = xmreg::evo_amount_to_str(sum_fees, "{:0.9f}", false);

    // get evo in the block reward
    context["blk_reward"] = xmreg::evo_amount_to_str(txd_coinbase.evo_outputs - sum_fees, "{:0.9f}");
                ...
                bool tx_prove = false)
{
    ...
    {
        return string("Evolution address not provided!");
    }
    ...
    if (!xmreg::parse_str_address(evo_address_str, address_info, nettype))
    {
...
//        }
    ...
    mstch::map context {
            ...
            {"tx_fee"               , xmreg::evo_amount_to_str(txd.fee, "{:0.9f}", true)},
            {"blk_timestamp"        , blk_timestamp},
...
//             << pod_to_hex(tx_pubkey) << '\n'  << '\n';
        ...
        outputs.push_back(mstch::map {
                ...
                {"amount"                , xmreg::evo_amount_to_str(outp.second)},
    ...
    for (const txin_to_key &in_key: input_key_imgs)
    {
        ...
        inputs.push_back(mstch::map{
                ...
                {"key_image_amount", xmreg::evo_amount_to_str(in_key.amount)},
...
//                        }
                ...
                found_outputs.push_back(mstch::map {
                        ...
                        {"amount"          , xmreg::evo_amount_to_str(amount)}
                });
...
//                                << endl;
    ...
    context["sum_evo"]           = xmreg::evo_amount_to_str(sum_evo);

    context.emplace("inputs", inputs);

    context["show_inputs"]   = show_key_images;
    context["inputs_no"]     = static_cast<uint64_t>(inputs.size());
    context["sum_mixin_evo"] = xmreg::evo_amount_to_str(sum_mixin_evo, "{:0.9f}", false);
    ...
    context["possible_spending"] = xmreg::evo_amount_to_str(possible_spending, "{:0.9f}", false);
        ...
        if (r)
        {
            ...
            for (const ::tools::wallet2::tx_construction_data &tx_cd: exported_txs.txes)
            {
                ...
                mstch::map tx_cd_data {
                        ...
                        {"change_amount"      , xmreg::evo_amount_to_str(tx_change.amount)},
                        {"has_payment_id"     , (payment_id  != null_hash)},
                        {"has_payment_id8"    , (payment_id8 != null_hash8)},
                        {"payment_id"         , pid_str},
                        {"payment_id8"        , pid8_str},
                ...
                for (const tx_destination_entry &a_dest: tx_cd.splitted_dsts)
                {
                    mstch::map dest_info {
                            ...
                            {"dest_amount"   , xmreg::evo_amount_to_str(a_dest.amount)}
                    };
                ...
                for (size_t i = 0; i < no_of_sources; ++i)
                {
                    ...
                    mstch::map single_dest_source {
                            {"output_amount"              , xmreg::evo_amount_to_str(tx_source.amount)},
                ...
                tx_cd_data.insert({"sum_outputs_amounts" ,
                                   xmreg::evo_amount_to_str(sum_outputs_amounts)});
...
//                serialization::serialize(ba, tx_from_blob);
        ...
        for (tools::wallet2::pending_tx &ptx: ptxs)
        {
            ...
            if (ptx.construction_data.change_dts.amount > 0)
            {
                destination_addresses.push_back(
                        mstch::map {
                                ...
                                {"dest_amount"    ,
                                        xmreg::evo_amount_to_str(ptx.construction_data.change_dts.amount)},
                                {"is_this_change" , true}
                        }
                );
            ...
            tx_context["outputs_evo_sum"] = xmreg::evo_amount_to_str(outputs_evo_sum);
            ...
            for (size_t i = 0; i < outputs.size(); ++i)
            {
                ...
                if (parse_amount(output_amount, out_amount_str))
                {
                    if (output_amount == 0)
                    {
                        out_amount_str = xmreg::evo_amount_to_str(real_ammounts.at(i));
                    }
                }
            }
            ...
            tx_context["inputs_evo_sum"] = xmreg::evo_amount_to_str(inputs_evo_sum);
            ...
            for (mstch::node &input_node: inputs)
            {
                ...
                amount = xmreg::evo_amount_to_str(real_amounts.at(input_idx));
...
show_checkcheckrawoutput(string raw_data, string viewkey_str)
{
    ...
    for (const tools::wallet2::transfer_details &td: outputs)
    {
        ...
        mstch::map output_info {
                ...
                {"amount"              , xmreg::evo_amount_to_str(evo_amount)},
    ...
    if (total_evo > 0)
    {
        ...
        context["total_evo"] = xmreg::evo_amount_to_str(total_evo);
    }
...
search(string search_text)
{
    ...
    // check if Evolution address is given based on its length
    ...
    // check if integrated Evolution address is given based on its length
...
json_rawblock(string block_no_or_hash)
{
    ...
    // get raw tx json as in Evolution
             ...
             bool tx_prove = false)
{
    ...
    {
        j_response["status"]  = "error";
        j_response["message"] = "Evolution address not provided";
        return j_response;
    }
    ...
    {
        j_response["status"]  = "error";
        j_response["message"] = "Cant parse Evolution address: " + address_str;
        return j_response;
...
json_networkinfo()
{
    ...
    if (!get_evolution_network_info(j_info))
    {
        j_response["status"]  = "error";
        j_response["message"] = "Cant get Evolution network info";
        return j_response;
    }
...
json_emission()
{
    ...
    else
    {
        ...
        string emission_blk_no   = std::to_string(current_values.blk_no - 1);
        string emission_coinbase = evo_amount_to_str(current_values.coinbase, "{:0.9f}");
        string emission_fee      = evo_amount_to_str(current_values.fee, "{:0.9f}", false);
...
json_version()
{
    ...
    j_data = json {
            ...
            {"evolution_version_full"  , string {EVOLUTION_VERSION_FULL}},
            {"api"                 , EVOLUTIONEXPLORER_RPC_VERSION},
...
construct_tx_context(transaction tx, uint16_t with_ring_signatures = 0)
{
    ...
    mstch::map context {
            ...
            {"tx_size"               , fmt::format("{:0.4f}", tx_size)},
            {"tx_fee"                , xmreg::evo_amount_to_str(txd.fee, "{:0.9f}", false)},
            {"tx_fee_nano"           , xmreg::evo_amount_to_str(txd.fee*1e9, "{:0.4f}", false)},
    ...
    for (const txin_to_key &in_key: txd.input_key_imgs)
    {
        ...
        inputs.push_back(mstch::map {
                ...
                {"amount"       , xmreg::evo_amount_to_str(in_key.amount)},
    ...
    context["inputs_evo_sum"]           = xmreg::evo_amount_to_str(inputs_evo_sum);
    ...
    context["outputs_evo_sum"] = xmreg::evo_amount_to_str(outputs_evo_sum);
...
bool
get_evolution_network_info(json &j_info)
{
...
get_footer()
{
    ...
    static const mstch::map footer_context {
            ...
            {"git_branch_name"     , string {GIT_BRANCH_NAME}},
            {"evolution_version_full"  , string {EVOLUTION_VERSION_FULL}},
            {"api"                 , std::to_string(EVOLUTIONEXPLORER_RPC_VERSION_MAJOR)
                                     + "."
                                     + std::to_string(EVOLUTIONEXPLORER_RPC_VERSION_MINOR)},
    };
